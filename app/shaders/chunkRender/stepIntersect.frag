...
const float INTERSECTION_PRECISION = 0.5;        // precision of the intersection
...
vec2 stepIntersect( vec3 ro , vec3 rd ){
  ...
  for( int i = 0; i < NUM_OF_TRACE_STEPS; i++ ){
    ...
    p += rd * STEP_SIZE / float(NUM_OF_TRACE_STEPS);

    float val = map( p );

    lum += val;
    stepID = id / NUM_OF_TRACE_STEPS;
